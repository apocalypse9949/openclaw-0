...
export function ensureMemoryIndexSchema(params: {
  ...
  if (params.cacheEnabled) {
    const table = quoteIdentifier(params.embeddingCacheTable);
    params.db.exec(`
    ...
    `);
    params.db.exec(
      `CREATE INDEX IF NOT EXISTS idx_embedding_cache_updated_at ON ${table}(updated_at);`,
    );
  }
  ...
  if (params.ftsEnabled) {
    try {
      ...
      const table = quoteIdentifier(params.ftsTable);
      params.db.exec(
