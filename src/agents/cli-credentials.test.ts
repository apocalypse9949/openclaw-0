...
function mockClaudeCliCredentialRead() {
  execFileSyncMock.mockImplementation((binary: unknown, args: unknown[]) => {
    if (String(binary) === "security" && args.map(String).includes("find-generic-password")) {
      return JSON.stringify({
        claudeAiOauth: {
...
describe("cli credentials", () => {
  ...
  it("reads Codex credentials from keychain when available", () => {
    ...
    execFileSyncMock.mockImplementation((binary: unknown, args: unknown[]) => {
      ...
      expect(argsStr).toContain("Codex Auth");
      expect(argsStr).toContain(accountHash);
      return JSON.stringify({
        tokens: {
  ...
  it("uses Codex auth.json fallback expiry when file mtime has fractional milliseconds", () => {
    ...
    try {
      const creds = readCodexCliCredentials({ platform: "linux", execFileSync: execFileSyncMock });
