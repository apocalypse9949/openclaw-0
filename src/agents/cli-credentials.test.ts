...
describe("cli credentials", () => {
  ...
  it("reads Codex credentials from keychain when available", () => {
    ...
    execFileSyncMock.mockImplementation((file: unknown, args: unknown) => {
      ...
      if (!cmdArgs.includes("Codex Auth")) { return undefined; }
      expect(cmdArgs).toContain("find-generic-password");
      const hasAccount = cmdArgs.some(arg => arg.startsWith('cli|')); expect(hasAccount).toBe(true);
      return JSON.stringify({
        tokens: {
