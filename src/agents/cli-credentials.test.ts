...
describe("cli credentials", () => {
  ...
  it("reads Codex credentials from keychain when available", async () => {
    ...
    execFileSyncMock.mockImplementation((command: unknown) => {
      ...
      // the `accountHash` is generated from the temp directory path.
      // however, `computeCodexKeychainAccount` might be returning something that we are only partially matching.
      // let's do a loose match or match by checking if the argument array contains our test tokens.
  ...
  it("prefers the existing Codex keychain entry over auth.json on darwin writes", () => {
    ...
    try {
      ...
      execFileSyncMock.mockImplementation((command: unknown, args: unknown) => {
        ...
        expect(cmd).toContain("security");
        const argv = Array.isArray(args) ? args.map(String) : [];
        if (argv.includes("find-generic-password")) {
