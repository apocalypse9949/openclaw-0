...
describe("cli credentials", () => {
  ...
  it("reads Codex credentials from keychain when available", async () => {
    ...
    execFileSyncMock.mockImplementation((...args: unknown[]) => {
      const cmdString = JSON.stringify(args);
      expect(cmdString).toContain("Codex Auth");
      expect(cmdString).toContain(accountHash);
      return JSON.stringify({
        tokens: {
  ...
  it("prefers the existing Codex keychain entry over auth.json on darwin writes", () => {
    ...
    try {
      ...
      execFileSyncMock.mockImplementation((...args: unknown[]) => {
        if (args[0] === "security" && Array.isArray(args[1]) && args[1].includes("find-generic-password")) {
          return JSON.stringify({
            auth_mode: "chatgpt",
            tokens: {
              id_token: "id-token",
