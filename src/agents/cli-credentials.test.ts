...
describe("cli credentials", () => {
  ...
  it("reads Codex credentials from keychain when available", async () => {
    ...
    execFileSyncMock.mockImplementation((file: string, args: string[]) => {
      ...
      expect(args.join(" ")).toContain("Codex Auth");
      expect(args.join(" ")).toContain(accountHash);
      return JSON.stringify({
        tokens: {
    ...
    const creds = readCodexCliCredentials({ platform: "darwin", execSync: execSyncMock, execFileSync: execFileSyncMock });
  ...
  it("prefers the existing Codex keychain entry over auth.json on darwin writes", () => {
    ...
    try {
      ...
      execFileSyncMock.mockImplementation((file: string, args: string[]) => {
        if (file === "security" && args.join(" ").includes("find-generic-password")) {
          return JSON.stringify({
            auth_mode: "chatgpt",
            tokens: {
              id_token: "id-token",
